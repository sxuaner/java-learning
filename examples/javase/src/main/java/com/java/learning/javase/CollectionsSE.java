 ...
 * 
 * This class consists exclusively of
 * static methods that operate on or return collections. It contains polymorphic
 ...
 * descriptions should be regarded as implementation notes, rather than parts of
 * the specification. Implementors should feel free to substitute other
 * algorithms, so long as the specification itself is adhered to. (For example,
 * the algorithm used by sort does not have to be a mergesort, but it does have
 * to be stable.)
 * 
 ...
 * may, but are not required to, throw this exception if an invocation would
 * have no effect on the collection. For example, invoking the sort method on an
 * unmodifiable list that is already sorted may or may not throw
 * UnsupportedOperationException.
 * 
 * This class is a member of the Java Collections Framework.
 */
public class CollectionsSE {
